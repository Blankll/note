...
![](../../statics/rocketmq/同步刷盘和异步刷盘.png)

#### 1）同步刷盘

在返回写成功状态时，消息已经被写入磁盘。具体流程是，消息写入内存的PAGECACHE后，立刻通知刷盘线程刷盘， 等待刷盘完成，刷盘线程执行完成后唤醒等待的线程，返回消息写成功的状态。

#### 2）异步刷盘

在返回写成功状态时，消息可能只是被写入了内存的PAGECACHE，写操作的返回快，吞吐量大；当内存里的消息量积累到一定程度时，统一触发顺序写磁盘动作，快速写入。
...
![](../../statics/rocketmq/消息发送高可用设计.jpg)
...
![](../../statics/rocketmq/复制刷盘.png)

实际应用中要结合业务场景，合理设置刷盘方式和主从复制方式， 尤其是SYNC_FLUSH方式，由于频繁地触发磁盘写动作，会明显降低 性能。通常情况下，应该把Master和Save配置成ASYNC_FLUSH的刷盘 方式，主从之间配置成SYNC_MASTER的复制方式，这样即使有一台 机器出故障，仍然能保证数据不丢，是个不错的选择。<font color="red">同步双写，异步刷盘</font>
...
![](../../statics/rocketmq/Topic路由信息.png)
...
![](../../statics/rocketmq/Broker故障延迟机制核心类.png)
...
![](../../statics/rocketmq/MappedFile.png)
...
![](../../statics/rocketmq/TransientStorePool.png)
...
![](../../statics/rocketmq/构建消息消费队列和索引文件.png)
...
![](../../statics/rocketmq/DispatchRequest.png)
...
![](../../statics/rocketmq/消息分发到消息消费队列.png)
...
![](../../statics/rocketmq/消息分发到索引文件.png)
...
![](../../statics/rocketmq/GroupCommitRequest.png)
...
![](../../statics/rocketmq/DefaultMQPushConsumer.png)
...
![](../../statics/rocketmq/pullMessageService实现机制.png)
...
![](../../statics/rocketmq/PullRequest.png)
...
![](../../statics/rocketmq/ProcessQueue.png)
...
![](../../statics/rocketmq/消息服务端Broker组装消息.png)
...
![](../../statics/rocketmq/消息拉取客户端处理消息.png)
...
![](../../statics/rocketmq/PullStatus.png)
...
![](../../statics/rocketmq/ConsumeMessageService.png)
